...
\noindent
\underline{Self-note:} line code $\rightarrow$ là quy tắt để máy tính truyền tín hiệu lên dây dẫn sao cho bên nhận có thể hiểu được.
...
\begin{itemize}
    \item \textbf{Pros:} hiệu quả cao $\rightarrow$ gửi 10 bit/s thì tín hiệu chỉ cần thay đổi tối đa 10 lần/s (10 Baud).
    \item \textbf{Cons:} đồng bộ kém (clock recovery problem) $\rightarrow$ gửi một dãy số 1 dài, đường dây sẽ giữ mức điện áp cao mãi $\Rightarrow$ bên nhận không biết đâu là bit thứ nhất, đâu là bit thứ hai.
\end{itemize}
\medskip
...
\begin{itemize}
    \item \textbf{Pros:} tự đồng bộ (self-synchronizing) $\rightarrow$ giữa mỗi bit luôn luôn có một cú nhảy (lên hoặc xuống) $\Rightarrow$ máy tính nhận biết ngay lập tức vị trí các bit $\Rightarrow$ không bao giờ bị mất đồng bộ dù có gửi cả đống số 1 liên tiếp.
    \item \textbf{Cons:} hiệu quả giảm một nửa; để gửi 10 bit/s thì tín hiệu phải thay đổi 20 lần/s (20 Baud) $\Rightarrow$ một bit cần 2 trạng thái điện áp (nửa đầu thấp, nửa sau cao) $\Rightarrow$ tốn băng thông gấp đôi so với NRZ.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Baseband} (dải cơ sở) $\rightarrow$ truyền tín hiệu nguyên bản (transmitted as is); tín hiệu số (xung vuông) được đẩy trực tiếp lên dây $\Rightarrow$ cần có data encoding để quy định rõ các mức điện áp 0 hay 1 $\rightarrow$ thường dùng cho mạng LAN hoặc kết nối ngắn.
    \item \textbf{Broadband} (dải rộng) $\rightarrow$ tín hiệu số được biến đổi (modulate) thành analog trước khi truyền đi $\Rightarrow$ cho phép chia đường truyền thành nhiều kênh tần số khác nhau (nhiều luồng dữ liệu có thể truyền cùng một lúc) $\rightarrow$ dùng cho truyền dẫn quãng dài như truyền hình cáp hay 4G chẳng hạn.
\end{itemize}
