...
\begin{mybox}{General task}
Enable communication among participants.
\end{mybox}
...
\sidebox[-4cm]{Self-note:}{$\Rightarrow$ Mạng peer-to-peer là tụi nó cứ liên tục swap role cho nhau, tụi nó ở ngang hàng nhau, không thằng nào hơn role thằng nào cả.}
...
\sidebox[-2cm]{Chú ý:}{Các thuật ngữ như "server", "client", "peer" chỉ dùng cho network và không đụng đến phần cứng.}
...
\sidebox[-6cm]{Self-note:}{Tại sao lại phải cần check coi có đúng "semantic" (nghĩa là xem coi nó nghe có hợp lý) không? \\ $\rightarrow$ Thử tưởng tượng ha, câu "tôi ăn đá" vẫn đúng cấu trúc ngữ pháp, tuy nhiên là trên thực tế câu này nghe rất vô lý $\Rightarrow$ chính vì thế nên mới phải check semantics.}
...
\end{itemize} $\Rightarrow$ Nói tóm lại: đây là network có vùng phủ nhỏ (few meters), thường có trong các điện thoại thông minh.
}

\sidebox[-3cm]{Độ phủ sóng:}{LAN < MAN < WAN}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.4\textwidth]{assets/chap_1/unicast.png}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.4\textwidth]{assets/chap_1/broadcast.png}
\end{figure}

\sidebox[-5cm]{Self-note:}{Unicast và broadcast gửi cho từng người một, khác với multicast và anycast là gửi cho nhóm $\rightarrow$ khác nhau giữa gửi không trong nhóm và gửi trong nhóm.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\textwidth]{assets/chap_1/multicast.png}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\textwidth]{assets/chap_1/anycast.png}
\end{figure}
\medskip

\sidebox[-11cm]{Self-note:}{Multicast và anycast ở đây chỉ gửi cho nhóm, nhưng multicast thì gửi mọi người trong nhóm, còn anycast thì chỉ gửi cho một người bất kỳ trong nhóm thôi.}
...
\sidebox[-3cm]{Self-note:}{Overhead nghĩa là phần tốn thêm để được truyền đi đúng cách, là mấy cái như header, trailer của gói tin chẳng hạn.}
...
\sidebox[-6.2cm]{Self-note:}{Anisotropy nghĩa là tính định hướng.}

\sidebox[-4cm]{Self-note:}{Endpoint có nghĩa là điểm giao tiếp.}
...
\sidebox[-2cm]{Self-note:}{Goodput $\le$ Throughput}
...
\begin{mybox}{Propagation delay}
\begin{equation*}
    \text{Propagation delay} = \frac{\text{Distance}}{\text{Speed of light} \times \text{Velocity factor}}
\end{equation*}
\end{mybox}
...
\begin{mybox}{Transmission delay}
\begin{equation*}
...
\sidebox[-1cm]{Self-note:}{Reference model nghĩa là mô hình tham chiếu }
...
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}
        \node[inner sep=0] (img) {\includegraphics[width=0.5\textwidth]{assets/chap_1/philo_trans_secret.png}};
        \draw[->, ultra thick]
...
\noindent
\textbf{TCP:} Transmission Control Protocol $\rightarrow$ like Certified Mail: guarantees that the recipient gets the package, and send receipt back $\Rightarrow$ if lost, it will re-send.

\begin{mybox}{Three-way Handshake}
\begin{equation*}
...
\noindent
\textbf{UDP:} User Datagram Protocol $\rightarrow$ like sending post card: drop in mailbox and hope it arrives $\Rightarrow$ faster and cheaper, but no confirmation receipt, and may lost.
\medskip

\sidebox[-5cm]{Self-note:}{IP nghĩa là Internet Protocol.}
\medskip
...
\begin{itemize}
    \item Application Layer: contains protocols that interact directly with programs $\Rightarrow$ format the actual "payload".
    \item Transport Layer: connects applications on different devices $\Rightarrow$ chops data into manageable pieces called segments.
    \item Internet Layer: handles logical addressing and routing; pack the segments into packets.
    \item Link Layer: handles physical transmission errors and control access to the wire/air; uses physical address (MAC address).
\end{itemize}
...
\begin{itemize}
    ...
    \item Checksum: kiểm tra xem file có lỗi gì khi tải về không (thường thấy dưới dạng dãy số SHA).
\end{itemize}
}
...
\sidebox[-4.75cm]{Self-note:}{OSI nghĩa là Open Systems Interconnection $\rightarrow$ là một khung khái niệm chia các chức năng truyền thông mạng thành 7 lớp.}

\subsection{Physical Layer}

\noindent
$\rightarrow$ \underline{Main concept:} raw signal with 0s and 1s.
...
\noindent
$\rightarrow$ \underline{Main concept:} group into frames, handle transmission errors (with checksums), specifies physical network address (MAC address).
\medskip
...
\noindent
$\rightarrow$ \underline{Main concept:} connects different neworks together.
\medskip
...
\noindent
$\rightarrow$ \underline{Main concept:} ensures the network get the correct application.
\medskip
...
\noindent
$\rightarrow$ \underline{Main concept:} dialogue control: setup, manage, and terminate connections (sessions).
\medskip
...
\sidebox[-4cm]{Self-note:}{Trong TCP/IP, người ta cho cái layer này vào trực tiếp cái Application Layer, ví dụ như dùng cookie để maintain session.}
...
\noindent
\underline{The core function:} Formatting and Syntax $\rightarrow$ to define the format (presentation) of the data.
...
\sidebox[-3cm]{Self-note:}{DNS nghĩa là Domain Name Server.}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{assets/chap_1/dns_illust.jpg}
\end{figure}
\medskip
...
\sidebox[-7cm]{Tóm tắt DNS:}{Client $\rightarrow$ Root $\rightarrow$ TLD $\rightarrow$ Authoritative $\rightarrow$ nhận IP.}
...
\sidebox[-3cm]{Self-note:}{Mesh network nghĩa là mạng lưới.}
